"""Tests for TestSupervisor (TestingSupervisor)."""
...
class TestTestSupervisorInit:
    def test_init(self):
        sup = TestSupervisor()
        assert sup.name == "TestSupervisor"
        assert sup.domain == "testing"
        assert sup.budget_inr == 4.0
        assert len(sup.agents) == 3
        assert not sup.is_budget_exhausted


class TestTestSupervisorRouting:
    @pytest.mark.asyncio
    async def test_routes_unit_run(self, supervisor):
        task = _make_task("TEST_UNIT_RUN")
        result = await supervisor.dispatch(task)
        assert result["status"] == "completed"

    @pytest.mark.asyncio
    async def test_routes_integration_run(self, supervisor):
        task = _make_task("TEST_INTEGRATION_RUN")
        result = await supervisor.dispatch(task)
        assert result["status"] == "completed"

    @pytest.mark.asyncio
    async def test_routes_coverage_check(self, supervisor):
        task = _make_task("TEST_COVERAGE_CHECK")
        result = await supervisor.dispatch(task)
        assert result["status"] == "completed"

    @pytest.mark.asyncio
    async def test_routes_regression(self, supervisor):
        task = _make_task("TEST_REGRESSION")
        result = await supervisor.dispatch(task)
        assert result["status"] == "completed"

    @pytest.mark.asyncio
    async def test_fallback_on_unknown(self, supervisor):
        task = _make_task("UNKNOWN_TEST_TASK")
        result = await supervisor.dispatch(task)
        assert result["status"] == "completed"
    ...
    async def test_budget_exhausted_rejection(self, supervisor):
        ...
        task = _make_task("TEST_UNIT_RUN", budget_inr=1.0)
        result = await supervisor.dispatch(task)
    ...
    async def test_register_agent_replaces_stub(self, supervisor):
        ...
        supervisor.register_agent("UnitTestAgent", CustomAgent())
        task = _make_task("TEST_UNIT_RUN")
        result = await supervisor.dispatch(task)
        assert result["status"] == "completed"
        assert len(calls) == 1
